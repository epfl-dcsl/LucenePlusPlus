    ...
    try {
        ...
        for (int32_t i = 0; i < argc; ++i) {
            if (strcmp(argv[i], "-index") == 0) {
                indices.push_back(StringUtils::toUnicode(argv[i + 1]));
                ++i;
        ...
        std::vector<RAMDirectoryPtr> rams;
        std::vector<IndexReaderPtr> ireaders;
        std::vector<SearcherPtr> vsearchers;
        ...
        for (int i = 0; i < indices.size(); i++) {
            ...
            searchers = newCollection<SearchablePtr>(searcher);
            searchers.add(searcher);
            rams.push_back(ram);
            ireaders.push_back(reader);
            vsearchers.push_back(searcher);
        }
        ...
        SearcherPtr searcher = newLucene<ParallelMultiSearcher>(searchers);
        ...
        while (true) {
            ...
            if (repeat > 0) { // repeat and time as benchmark
                int64_t starttime = GetUsec();
                for (int32_t i = 0; i < repeat; ++i) {
                    searcher->search(query, FilterPtr(), 100);
                }
                int64_t stoptime = GetUsec();
                int64_t total = stoptime - starttime;

                std::wcout << ((double)(total)) / ((double)repeat) << "\n";
            }
        }
        for (int i = 0; i < ireaders.size(); i++) {
            ireaders[i]->close();
        }
