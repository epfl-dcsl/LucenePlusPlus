...
int main(int argc, char* argv[]) {
        ...
        std::wcout << L"Usage: termquery.exe <source dir> [-index dir] [-field f] [-repeat n] [-queries file] [-raw] ";
    ...
    if (!FileUtils::isDirectory(sourceDir)) {
        std::wcout << L"Source directory does not exit: ";
    }


    uint64_t beginIndex = MiscUtils::currentTimeMillis();
    RAMDirectoryPtr ramPtr = newLucene<RAMDirectory>();
    try {
        ...
        IndexWriterPtr writer = newLucene<IndexWriter>(ramPtr, newLucene<StandardAnalyzer>(LuceneVersion::LUCENE_CURRENT), true, IndexWriter::MaxFieldLengthLIMITED);
        std::wcout << L"Indexing to RAMDirectory\n";
        ...
        writer->close();
        std::wcout << L"Total time writing: " << indexDuration + optimizeDuration << L" milliseconds\n";

        IndexReaderPtr reader = IndexReader::open(ramPtr, true);
